...
impl<D: DualNum> SE3<D> {
    pub fn to_matrix(&self) -> na::Matrix4<D> {
        ...
        mat.fixed_view_mut::<3, 1>(0, 3).copy_from(&self.xyz.0);
        mat
    }

    pub fn from_matrix(mat: &na::Matrix4<D>) -> Self {
...
impl<D: DualNum> LieGroup<D> for SE3<D> {
    ...
    fn exp(xi: &VectorD<D>) -> Self {
        let xi_rot = na::dvector![xi[0].clone(), xi[1].clone(), xi[2].clone()];
    ...
    fn log(&self) -> VectorD<D> {
        ...
        let V: na::SMatrix<D, 3, 3> = if w.clone().abs() < D::from(1e-6) {
            I + &wx / D::from(2.0) + &wx * &wx / D::from(6.0) + &wx * &wx * &wx / D::from(24.0)
        } else {
            let A = w.clone().sin() / w.clone();
            let B = (D::from(1.0) - w.clone().cos()) / (w.clone() * w.clone());
        ...
        let xyz = Vector(Vinv * self.xyz.0.clone());
    ...
    fn wedge(xi: &VectorD<D>) -> na::DMatrix<D> {
        let mut mat = na::DMatrix::<D>::zeros(4, 4);
        mat[(0, 1)] = -xi[2].clone();
        mat[(0, 2)] = xi[1].clone();
        mat[(1, 0)] = xi[2].clone();
        mat[(1, 2)] = -xi[0].clone();
...
mod tests {
    ...
    #[test]
    fn exp_log() {
        let xi = na::dvector![0.1, 0.2, 0.3, 1.0, 2.0, 3.0];
        let se3 = SE3::exp(&xi);
        let xi_hat = se3.log();
        println!("{} {}", xi, xi_hat);
    ...
    fn test_jacobian() {
        ...
        let v = na::dvector![0.1, 0.2, 0.3, 1.0, 2.0, 3.0];
        let (x, dx) = jacobian(compute, v.clone());

        assert!((x - v).norm() < 1e-6);
        assert!((na::DMatrix::identity(6, 6) - dx).norm() < 1e-6);
    }
}
